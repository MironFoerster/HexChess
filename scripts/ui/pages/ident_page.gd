...
func _ready() -> void:
	GameManager.ident_processed.connect(_on_ident_processed)
...
func _on_nickname_submit_button_pressed() -> void:
	GlobalNetworking.nickname_player(nickname.text)

func _on_login_submit_button_pressed() -> void:
	GlobalNetworking.login_player(login_username.text, login_password.text)

func _on_register_submit_button_pressed() -> void:
	if register_password.text == register_password_repeat.text:
		GlobalNetworking.register_player(register_username.text, register_password.text)
	else:
